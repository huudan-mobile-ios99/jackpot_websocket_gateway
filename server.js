...
function connect() {
  ...
  var client = new WebSocket.Client('ws://192.168.100.202/Interfaces/Media/JackpotsGateway?COMPUTERNAME=Application');
...
startServerAndPublishData()
    .then(() => {
        ...
        setInterval(checkAndEmitDefaultJackpotHit,20000); // Check every 10 seconds
    })
    .catch(error => {
  ...
  parser.parseString(msg, function (err, result) {
    try {
      if ("JackpotHit" in result) {
        ...
        console.log('emit Dynamic jackpothit');
        io.emit('jackpotHit', {
